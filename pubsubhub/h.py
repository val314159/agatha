...
def _():
    ...
    if result.get('status') == 'ok' and 'session_token' in result:
        response.set_cookie('session', result['session_token'],
                            path='/', httponly=True, samesite='lax')
        logger.info("SIGN IN from %r (device %r) -> session %r",
                    email, device_id, result['session_token'])
    else:
        logger.info("FAILED SIGN IN from %r (device %r)",
                    email, device_id)
        pass
    return result
